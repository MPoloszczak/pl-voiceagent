...
#REMOVE IN PROD-
        # ------------------------------------------------------------------
        # TEMPORARY DIAGNOSTIC – log the IAM username and generated SigV4
        # token so we can confirm we're sending the expected credentials.
        # This call produces a short-lived token (≤15 min) that will be
        # regenerated by redis-py when needed, so calling it once here is safe.
        # ------------------------------------------------------------------
        try:
            _iam_user_dbg, _iam_token_dbg = provider.get_credentials()
            logger.info(
                "🪪 Using IAM user '%s' with SigV4 token (first 120 chars): %s…",
                _iam_user_dbg,
                _iam_token_dbg[:120],
            )
